preprocess_data <- function(data, dataType, center = NULL, scale = NULL) {
    ## 資料清理
    # 0 確認資料幾列幾欄
    dimensions <- dim(data)
    ...
    if (dataType == "training") {
      ...
      data_train$Fare[is.na(data_train$Fare)] <- mean(data_train$Fare, na.rm = TRUE)
      data_train$Age[is.na(data_train$Age)] <- mean(data_train$Age, na.rm = TRUE)
    }
    ...
    if (dataType == "training") {
      scaled_data <- scale(data_train_subset_scale[, -which(names(data_train_subset_scale) == "Survived")])
      data_train_subset_scale[, -which(names(data_train_subset_scale) == "Survived")] <- as.data.frame(scaled_data)
    } else {
      scaled_data <- scale(data_train_subset_scale, center = center, scale = scale)
      data_train_subset_scale <- as.data.frame(scaled_data)
    }

    attr(data_train_subset_scale, "scaled:scale") <- attr(scaled_data, "scaled:scale")
    attr(data_train_subset_scale, "scaled:center") <- attr(scaled_data, "scaled:center")
